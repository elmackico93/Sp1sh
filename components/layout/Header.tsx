...
import TerminalSyncIcon from '../scripts/TerminalSyncIcon';
...
export const Header = () => {
  ...
  // Handle cursor blinking - only blink when typing is complete
  useEffect(() => {
    const cursor = document.getElementById('cursor');
    if (!cursor) return;
    if (isTyping) {
  ...
  return (
    <header className="sticky top-0 z-50 bg-white dark:bg-gray-900 shadow-sm backdrop-blur-lg dark:shadow-gray-800/10 border-b border-gray-100 dark:border-gray-800">
      <div className="container mx-auto px-4 h-16 flex items-center justify-between">
        <button 
          onClick={rotateClaim} 
          className="flex items-center gap-2 my-2 sm:my-1.5 focus:outline-none logo-container"
        >
          <div className="flex items-center gap-2">
            ...
            >
              <span id="typed-claim" className="whitespace-nowrap text-left block">
                {animatedClaim}
              </span>
        ...
        <div className="flex items-center gap-2">
          ...
          <Link 
            href="/signin" 
            className="hidden md:block px-4 py-2 text-sm font-medium text-gray-700 dark:text-gray-300 hover:text-primary dark:hover:text-primary-light"
          >
            Sign In
          </Link>
          <Link 
            href="/add-script" 
            className="hidden md:flex items-center gap-1 px-4 py-2 text-sm font-medium text-white bg-primary hover:bg-primary-dark dark:bg-primary-light dark:hover:bg-primary dark:text-gray-900 rounded-full transition-colors"
          >
            <span>+</span>
